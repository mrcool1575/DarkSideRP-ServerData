...
function renderBoxes()
	...
	if(IsPedInAnyVehicle(playerPed, true) and not isBlackListedModel()) then
		...
		if isRadarExtended then
			...
			DrawAdvancedText(text_x, text_y, 0.005, 0.0028, 0.45,round(percent,1).."%", 255, 255, 255, 255, 0, 1)
		elseif(hud_form == 1) then
			local percent = (essence/0.142)*100
		
		if percent > 20 and percent <= 100 then
			DrawAdvancedText(text_x, text_y, 0.005, 0.0028, 0.45,round(percent,1).."%", 255, 255, 255, 255, 0, 1)
			elseif percent < 19 then 
			DrawAdvancedText(text_x, text_y, 0.005, 0.0028, 0.45,round(percent,1).."%", 255, 0, 0, 255, 0, 1)
		else
			local percent = (essence/0.142)*100
